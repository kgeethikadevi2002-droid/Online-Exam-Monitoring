import cv2
import sqlite3
from datetime import datetime
import time

# ========== 1. DATABASE SETUP ==========
DATABASE = "exam.db"
CANDIDATE_ID = "C101" # Change this to the logged-in candidate

def create_db():
    conn = sqlite3.connect(DATABASE)
    c = conn.cursor()
    # Create EventLog table if not exists
    c.execute('''CREATE TABLE IF NOT EXISTS EventLog(
        event_id INTEGER PRIMARY KEY AUTOINCREMENT,
        candidate_id TEXT,
        event_type TEXT,
        timestamp TEXT,
        remarks TEXT
    )''')
    conn.commit()
    conn.close()

def log_event(candidate_id, event_type, remarks):
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    conn = sqlite3.connect(DATABASE)
    c = conn.cursor()
    c.execute("INSERT INTO EventLog(candidate_id, event_type, timestamp, remarks) VALUES (?,?,?,?)",
              (candidate_id, event_type, timestamp, remarks))
    conn.commit()
    conn.close()
    print(f"[LOGGED] {event_type} at {timestamp}")

create_db()

# ========== 2. FACE DETECTION SETUP ==========
CASCADE_PATH = cv2.data.haarcascades + "haarcascade_frontalface_default.xml"
FACE_CASCADE = cv2.CascadeClassifier(CASCADE_PATH)
cam = cv2.VideoCapture(0)

if not cam.isOpened():
    print("ERROR: Cannot open webcam")
    exit()

print(f"Monitoring Started for {CANDIDATE_ID}... Press 'q' to Quit")

# ========== 3. VARIABLES FOR LOGIC ==========
face_was_present = True  # Track previous state to avoid duplicate logs

while True:
    ret, frame = cam.read()
    if not ret:
        break

    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    faces = FACE_CASCADE.detectMultiScale(gray, scaleFactor=1.3, minNeighbors=5)

    # ========== 4. CHECK FACE AND LOG EVENT ==========
    if len(faces) > 0:
        # FACE DETECTED
        status = "Face Detected"
        color = (0, 255, 0) # Green
        
        for (x, y, w, h) in faces:
            cv2.rectangle(frame, (x, y), (x + w, y + h), color, 2)
        
        face_was_present = True # Update state

    else:
        # FACE NOT DETECTED
        status = "Face Not Detected"
        color = (0, 0, 255) # Red

        # LOG ONLY WHEN STATUS CHANGES FROM PRESENT TO ABSENT
        if face_was_present == True:
            remarks = "Candidate face not visible in camera frame"
            log_event(CANDIDATE_ID, "Face Not Detected", remarks)
            face_was_present = False # Update state so we don't log again

    # ========== 5. DISPLAY INFO ON SCREEN ==========
    cv2.putText(frame, f"Candidate: {CANDIDATE_ID}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,255,255), 2)
    cv2.putText(frame, f"Status: {status}", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 2)
    cv2.putText(frame, f"Time: {datetime.now().strftime('%H:%M:%S')}", (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,255,255), 2)
    cv2.putText(frame, "Press 'q' to Quit", (10, 450), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)

    cv2.imshow("Exam Monitoring", frame)

    key = cv2.waitKey(1) & 0xFF
    if key == ord('q'):
        break

cam.release()
cv2.destroyAllWindows()